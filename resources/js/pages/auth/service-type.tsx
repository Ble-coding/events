...
export default function ServiceTypeManager() {
  ...
  const { types, flash, auth, allserviceItems, errors  } = usePage<PageProps>().props;
...
//   const { toast } = useToast();
  ...
  const filtered = useMemo(() => {
    ...
    const list = allserviceItems ?? types.data; // ✅ fallback sécurisé

    if (!term) return types.data;
  ...
  }, [search, types.data, allserviceItems]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!data.name.trim()) {
        setErrorMessage('Veuillez entrer un nom de type de service');
...
{!hasSearch && (   <div className="flex justify-center gap-2 mt-6">
